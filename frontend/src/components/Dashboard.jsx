...
function Dashboard({ user, onStartGame }) {
        ...
        const fetchMaps = async () => {
            try {
                ...
                if (!user.maps || user.maps.length === 0) {
                    ...
                    const sectorNames = ['Level 1: The Beginning', 'Level 2: The Core', 'Level 3: The Lab', 'Level 4: The Command'];
                    const merged = dbMaps.map((dbM, idx) => {
                        const userM = user.maps.find(um => um.id === dbM.id);

                        // Rule: Level is unlocked if it's the first one OR the previous one is completed
    ...
    const mapsToDisplay = availableMaps.length > 0 ? availableMaps : [
        { id: 'map1', title: 'Level 1: The Beginning', unlocked: true, completed: false, lastStage: 1, bestScore: 0 },
        { id: 'map2', title: 'Level 2: The Core', unlocked: false, completed: false, lastStage: 5, bestScore: 0 },
        { id: 'map3', title: 'Level 3: The Lab', unlocked: false, completed: false, lastStage: 9, bestScore: 0 },
        { id: 'map4', title: 'Level 4: The Command', unlocked: false, completed: false, lastStage: 13, bestScore: 0 }
    ];
    ...
    return (
        <div className="main-content animate-fade">
            <header style={{ marginBottom: '3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderBottom: '1px solid var(--border)', paddingBottom: '1.5rem' }}>
                <div>
                    <h1 className="glow-text" style={{ fontSize: '2.5rem', marginBottom: '0.5rem' }}>Dashboard</h1>
            ...
            <section style={{ marginBottom: '4rem' }}>
                <h2 style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '15px', textTransform: 'uppercase', letterSpacing: '2px', fontSize: '1.1rem' }}>
                    <Shield size={20} color="var(--neon-cyan)" /> Play Levels
                </h2>
            ...
            <section style={{ marginBottom: '4rem' }}>
                <h2 style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '15px', textTransform: 'uppercase', letterSpacing: '2px', fontSize: '1.1rem' }}>
                    <BarChart3 size={20} color="var(--neon-pink)" /> Player Analytics
                </h2>
