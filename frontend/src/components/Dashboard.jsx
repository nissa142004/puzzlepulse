...
function Dashboard({ user, onStartGame }) {
        ...
        const fetchMaps = async () => {
            try {
                ...
                if (!user.maps || user.maps.length === 0) {
                    ...
                    const sectorNames = ['Level 1: Outer Hull', 'Level 2: Engine Core', 'Level 3: Research Lab', 'Level 4: Command Deck'];
                    const merged = dbMaps.map((dbM, idx) => {
                        const userM = user.maps.find(um => um.id === dbM.id);

                        // Rule: World is unlocked if it's the first one OR the previous one is completed
    ...
    const mapsToDisplay = availableMaps.length > 0 ? availableMaps : [
        { id: 'map1', title: 'World 1: Outer Hull', unlocked: true, completed: false, lastStage: 1, bestScore: 0 },
        { id: 'map2', title: 'World 2: Engine Core', unlocked: false, completed: false, lastStage: 5, bestScore: 0 },
        { id: 'map3', title: 'World 3: Research Lab', unlocked: false, completed: false, lastStage: 9, bestScore: 0 },
        { id: 'map4', title: 'World 4: Command Deck', unlocked: false, completed: false, lastStage: 13, bestScore: 0 }
    ];
    ...
    return (
        <div className="main-content animate-fade">
            <header style={{ marginBottom: '3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderBottom: '1px solid var(--border)', paddingBottom: '1.5rem' }}>
                <div>
                    <h1 className="glow-text" style={{ fontSize: '2.5rem', marginBottom: '0.5rem' }}>Game Dashboard</h1>
            ...
            <section style={{ marginBottom: '4rem' }}>
                <h2 style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '15px', textTransform: 'uppercase', letterSpacing: '2px', fontSize: '1.1rem' }}>
                    <Shield size={20} color="var(--neon-cyan)" /> Available Maps
                </h2>
            ...
            <section style={{ marginBottom: '4rem' }}>
                <h2 style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '15px', textTransform: 'uppercase', letterSpacing: '2px', fontSize: '1.1rem' }}>
                    <BarChart3 size={20} color="var(--neon-pink)" /> Player Statistics
                </h2>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        ...
                        <p style={{ color: 'var(--text-dim)', fontSize: '0.9rem', maxWidth: '800px' }}>
                            Choose an active map. Complete 5 levels in each world to unlock the next area.
                            Your progress is capped at 15 total levels.
                        </p>
                    </div>
